...
namespace EasyGUI
{
    ...
    */
    ...
    class EasyGUI
    {
        ...
        //---------------------------------------------------------------------
        void DrawString(int X, int Y, string String, vector<int> TextColor, LPCWSTR Fount_Name, short Fount_Size, short Font_Width) noexcept//绘制文字 (方便制作GUI)
        {
    ...
    public:
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        template<class ValueClass, class CreateClassName>
        ValueClass GUI_Slider_Rainbow(vector<int>BlockPos, short LineRow, string Text, ValueClass StartValue, ValueClass EndValue, ValueClass& m_SliderValue) noexcept//绘制彩虹变色滑条
        {
            if (BlockPos[0] == 0 && BlockPos[1] == 0)return 0;//当无block则不进行绘制
            ValueClass ClassValueDetect = 0.1;
            POINT m_MousePos; GetCursorPos(&m_MousePos);//获取鼠标坐标
            ...
            stringstream ss; ss << fixed << setprecision(4) << m_SliderValue; ss >> m_SliderValue;//只保留4位小数点后数
            DrawRect(BlockPos[0] - 1 + 55, BlockPos[1] - 1 + (6 + 30 * LineRow), 230 + 2, 7, { 0,0,0 });//黑色外边框
            DrawGradientRect(BlockPos[0] + 55, BlockPos[1] + (6 + 30 * LineRow), 230, 5, { (int)floor(sin((float)GetTickCount64() / 2000 * 2 + 2) * 127 + 128), (int)floor(sin((float)GetTickCount64() / 2000 * 2 + 2 + 2) * 127 + 128), (int)floor(sin((float)GetTickCount64() / 2000 * 2 + 4 + 2) * 127 + 128) }, { (int)floor(sin((float)GetTickCount64() / 2000 * 2) * 127 + 128), (int)floor(sin((float)GetTickCount64() / 2000 * 2 + 2) * 127 + 128), (int)floor(sin((float)GetTickCount64() / 2000 * 2 + 4) * 127 + 128) }, false);//滑条
            if (DetectMousePos || OutSide)DrawGradientRect(SliderPos + BlockPos[0] + 55, BlockPos[1] + (6 + 30 * LineRow), 230 - SliderPos, 5, { 40,40,40 }, { 60,60,60 }, true);//滑条背景(前负责遮挡)
