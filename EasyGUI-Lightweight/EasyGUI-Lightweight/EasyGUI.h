...
namespace EasyGUI//控件
{
    ...
    */
    ...
    constexpr float constexpr_abs(float x) { return x < 0 ? -x : x; }
    static int m_fps = 0;
    static unsigned long long Tick_Old = 0;
    ...
    struct Vector4//用来储存颜色数据 RGBA
    {
        ...
        constexpr const Vector4 Sat(float saturation = 0.6, float grey = 0.025) const noexcept//饱和度设置
        {
            ...
            if (constexpr_abs(delta) < 0.00001f)h = 0; else if (constexpr_abs(max_val - rNorm) < 0.00001f) h = 60 * (((gNorm - bNorm) / delta) + (gNorm < bNorm ? 6 : 0));
            else if ((max_val - gNorm) < 0.00001f)h = 60 * (((bNorm - rNorm) / delta) + 2); else h = 60 * (((rNorm - gNorm) / delta) + 4);
            s = (constexpr_abs(max_val) < 0.00001f) ? 0 : (delta / max_val); s *= saturation / 1; if (s < 0)s = 0; else if (s > 1)s = 1;
    ...
    struct EasyGUI_IO
    {
        ...
        std::string WindowTitle = "";//全局GUI窗口标题
        Vector2 MousePos = { 0,0 };//GUI检测的鼠标坐标
        std::vector<int> GUIColor_Rainbow = { 0,0,0,0,0,0,0,0,0 };//全局GUI彩虹颜色
    };
    //---------------------------------------------------------------------------------------------------------------------------------------------------------

    static BOOL china_word2, china_word6; static Vector2 OldPos;//按下时坐标
    ...
    private:
        //-----------菜单风格
        std::string Global_EasyGUIFont = "Verdana";//全局字体名称
        int Global_EasyGUIFontSize = 13;//全局字体大小
        Vector4 Global_EasyGUIColor = { 255,255,255,250 };//全局china_word5 (包含窗口透明度)
        //------------------
        ...
        //------------------
        BOOL Mouse_Block_ = FALSE, Mouse_Slider_ = FALSE, Mouse_Move_ = FALSE;//防止控件函数之间冲突的判断变量
        //------------------
    public:
        //---------------------------------------------------------------------开发者函数 (方便制作GUI)
        ...
        //---------------------------------------------------------------------
        void In_DrawTriangle(const std::vector<int> & Pos_Point, Vector4 Color) noexcept//绘制三角形
        {
            Color = Color.Sat();
        ...
        //---------------------------------------------------------------------
        int In_DrawString(int X, int Y, std::string String, Vector4 Color, const std::string & Fount_Name, int Fount_Size, int Font_Width = FW_NORMAL) noexcept//绘制文字
        {
            Color = Color.Sat(); if (String.empty() || Fount_Size == 0)return 0; String = String.substr(0, 100);
            if (Fount_Name == "微软雅黑")Y -= Fount_Size / 7; else if (Fount_Size > 15)Y -= Fount_Size / 3;//平衡上下坐标
            BOOL Transform = TRUE; if (String.find("UTT") != std::string::npos) { String.erase(String.find("UTT"), 3); Transform = FALSE; }//转换UTF-8
            ...
            SIZE TextSize; GetTextExtentPoint32A(EasyGUI_DrawHDC, String.c_str(), String.length(), &TextSize); if (X >= 10000)X -= 10000 + TextSize.cx / 2; if (!X && !Y) { DeleteObject(FontPen); return TextSize.cx; }//X坐标大于10000时采用居中坐标
            ...
            else TextOutA(EasyGUI_DrawHDC, X, Y, String.c_str(), String.length());//不转换UTF-8直接绘制
            DeleteObject(FontPen);
            return TextSize.cx;
        }
        //---------------------------------------------------------------------
        int In_DrawString_Simple(int X, int Y, std::string String, Vector4 Color = { 255,255,255 }) noexcept//绘制简单文字
        {
            Color = Color.Sat(); if (String.empty())return 0; String = String.substr(0, 100);
            ...
            TextOutA(EasyGUI_DrawHDC, X + 1, Y + 1, String.c_str(), String.length());
            TextOutA(EasyGUI_DrawHDC, X - 1, Y - 1, String.c_str(), String.length());
            TextOutA(EasyGUI_DrawHDC, X + 1, Y - 1, String.c_str(), String.length());
            TextOutA(EasyGUI_DrawHDC, X - 1, Y + 1, String.c_str(), String.length());
            TextOutA(EasyGUI_DrawHDC, X + 1, Y, String.c_str(), String.length());
            TextOutA(EasyGUI_DrawHDC, X - 1, Y, String.c_str(), String.length());
            TextOutA(EasyGUI_DrawHDC, X, Y - 1, String.c_str(), String.length());
            TextOutA(EasyGUI_DrawHDC, X, Y + 1, String.c_str(), String.length());
            SetTextColor(EasyGUI_DrawHDC, RGB(Color.r, Color.g, Color.b));//文字颜色
            TextOutA(EasyGUI_DrawHDC, X, Y, String.c_str(), String.length());
            DeleteObject(FontPen);
            return TextSize.cx;
        }
        //---------------------------------------------------------------------
        ...
        {
            const unsigned long long Tick = GetTickCount64(); static unsigned long long OldTick = Tick;
        ...
        {
            if (fabs(Speed - 1) < 0.00001f)return Value;//无动画时直接返回
            ...
            if (Limit.x != 0 || Limit.y != 0) { if (Limit.x > ReturnValue)ReturnValue = (float)Limit.x; else if (Limit.y < ReturnValue)ReturnValue = (float)Limit.y; }
            return ReturnValue;
        }
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        void Global_Set_EasyGUI_Font(const std::string & FontName) noexcept { Global_EasyGUIFont = FontName; }//设置全局GUI字体
        void Global_Set_EasyGUI_FontSize(int FontSize) noexcept { Global_EasyGUIFontSize = FontSize; }//设置全局GUI字体大小
        void Global_Set_EasyGUI_Color(Vector4 MainColor) noexcept { Global_EasyGUIColor = MainColor; }//设置全局china_word5
        std::string Global_Get_EasyGUI_Font() noexcept { return Global_EasyGUIFont; }//获取全局GUI字体
        int Global_Get_EasyGUI_FontSize() noexcept { return Global_EasyGUIFontSize; }//获取全局GUI字体大小
        Vector4 Global_Get_EasyGUI_Color() noexcept { return Global_EasyGUIColor; }//获取全局china_word5
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        BOOL Window_Create(int Size_X, int Size_Y, std::string WindowTitle, BOOL IfTop = FALSE) noexcept//创建窗口
        {
            int china_word = WS_EX_LAYERED;
            if (IfTop)china_word = WS_EX_TOPMOST | WS_EX_LAYERED;
            ...
            RenderWindowM.lpszClassName = std::wstring(WindowTitle.begin(), WindowTitle.end()).c_str();//窗口类的名称，操作系统根据类的名称做映射
            RegisterClass(&RenderWindowM);//将这个窗体注册（告诉）到操作系统
            const HWND hWnd = CreateWindowEx(china_word, std::wstring(WindowTitle.begin(), WindowTitle.end()).c_str(), std::wstring(WindowTitle.begin(), WindowTitle.end()).c_str(), WS_POPUP, GetSystemMetrics(SM_CXSCREEN) / 2 - Size_X / 2, GetSystemMetrics(SM_CYSCREEN) / 2 - Size_Y / 2, Size_X, Size_Y, 0, 0, GetModuleHandle(0), 0);
            if (hWnd)//当创建成功
            {
        ...
        void Draw_GUI(BOOL ReverseColor = FALSE) noexcept//双缓冲结束绘制 (绘制最终返回图片)
        {
            ...
            //--------------------------------帧数计算
            m_fps++;
            const unsigned long long Tick = GetTickCount64(); Tick_Old = Tick;
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        void Window_SetAlpha(int WindowAlpha = 255) noexcept//修改GUI窗口整体阿尔法通道
        {
            if (WindowAlpha >= 255)WindowAlpha = 255; else if (WindowAlpha <= 0)WindowAlpha = 0;
            SetLayeredWindowAttributes(EasyGUI_WindowHWND, RGB(0, 0, 0), (unsigned char)WindowAlpha, LWA_ALPHA);
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        void Window_SetTitle(const std::string & WindowTitle) noexcept//修改GUI窗口标题
        {
            SetWindowTextA(EasyGUI_WindowHWND, WindowTitle.c_str());//修改窗口标题
        }
        std::string Window_GetTitle() noexcept//获取GUI窗口标题
        {
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        BOOL Window_Move(int Draw_Delay = 3, BOOL Limit_Edge = FALSE) noexcept//移动GUI窗口 (在GUI循环线程内加入此函数不需要添加延时函数来降低CPU占用)
        {
            //--------------------------------消息循环
            MSG MSG = { 0 }; if (GetMessage(&MSG, 0, 0, 0)) { TranslateMessage(&MSG); DispatchMessage(&MSG); }
            GetCursorPos(&EasyGUI_MousePos); GetWindowRect(EasyGUI_WindowHWND, &EasyGUI_WindowPos);//刷新鼠标窗口坐标
            ...
            {
                if (!Mouse_Block_ && In_KeyEvent(VK_LBUTTON) && !china_word2)//触发移动窗口机制
                {
                    if (china_word6) { OldPos = { EasyGUI_MousePos.x - EasyGUI_WindowPos.left ,EasyGUI_MousePos.y - EasyGUI_WindowPos.top }; china_word6 = FALSE; }
                    china_word2 = TRUE;//触发移动窗口事件变量
                }
                else if (china_word2 && In_KeyEvent(VK_LBUTTON))//移动窗口时
                {
                    ...
                    china_word2 = FALSE; china_word6 = TRUE; Mouse_Block_ = FALSE; Mouse_Move_ = FALSE;
                    if (!Mouse_Slider_)
                    {
                        if (In_KeyEvent(VK_LBUTTON))Sleep(1);
                        else Sleep(Draw_Delay);//降低CPU占用
                    }
                    else std::this_thread::sleep_for(std::chrono::nanoseconds(80));//纳秒休眠函数 (让滑条更加顺滑)
                    return FALSE;
                }
            }
        ...
        //-------------------------------------------------------------------------------------------------------------------------------------------以下是控件函数*
        void GUI_BackGround(int BackGround_StyleCode = 0) noexcept//绘制GUI窗口背景
        {
            const Vector2 Window_Size = { EasyGUI_WindowPos.right - EasyGUI_WindowPos.left ,EasyGUI_WindowPos.bottom - EasyGUI_WindowPos.top };
            std::vector<int> china_word4 = { 0,255,255,255,0,255,255,255,0 };
            std::vector<int> china_word5 = { 0,0,0,60,60,60,30,30,30,15,15,15,5,5,5,30,30,30 };
            if (BackGround_StyleCode == 0)//毛都没有
            {
                china_word4 = { 16,16,16,16,16,16,16,16,16 };
                china_word5 = { 0,0,0,60,60,60,30,30,30,15,15,15,15,15,15,Global_EasyGUIColor.r / 10,Global_EasyGUIColor.g / 10,Global_EasyGUIColor.b / 10 };
            }
            else if (BackGround_StyleCode == 1)
            {
                china_word4 = { 100,255,255,255,100,255,255,255,100 };
                china_word5 = { 0,0,0,60,60,60,30,30,30,15,15,15,5,5,5,Global_EasyGUIColor.r / 10,Global_EasyGUIColor.g / 10,Global_EasyGUIColor.b / 10 };
            }
            else if (BackGround_StyleCode == 2)
            {
                china_word4 = { 0,255,255,255,0,255,255,255,0 };
                china_word5 = { 0,0,0,60,60,60,30,30,30,15,15,15,3,3,3,Global_EasyGUIColor.r / 10,Global_EasyGUIColor.g / 10,Global_EasyGUIColor.b / 10 };
            }
            else if (BackGround_StyleCode == 3)//彩色变色渐变条
            {
                ...
                china_word5 = { 0,0,0,60,60,60,30,30,30,15,15,15,3,3,3,Global_EasyGUIColor.r / 10,Global_EasyGUIColor.g / 10,Global_EasyGUIColor.b / 10 };
                Global_EasyGUIColor = { china_word4[3],china_word4[4], china_word4[5] };
            }
            else if (BackGround_StyleCode == 4)//主题色渐变条
            {
                ...
                const auto Min = 50; for (int i = 0; i <= 8; ++i)if (china_word4[i] < Min)china_word4[i] = Min;//颜色最小值
                china_word5 = { 0,0,0,60,60,60,30,30,30,15,15,15,0,0,0,Global_EasyGUIColor.r / 10,Global_EasyGUIColor.g / 10,Global_EasyGUIColor.b / 10 };
            }
            In_DrawRect(0, 0, Window_Size.x, Window_Size.y, { china_word5[0], china_word5[1], china_word5[2] });
            In_DrawRect(1, 1, Window_Size.x - 2, Window_Size.y - 2, { china_word5[3], china_word5[4], china_word5[5] });
            In_DrawRect(2, 2, Window_Size.x - 4, Window_Size.y - 4, { china_word5[6], china_word5[7], china_word5[8] });
            In_DrawRect(5, 5, Window_Size.x - 10, Window_Size.y - 10, { china_word5[3], china_word5[4], china_word5[5] });
            In_DrawGradientRect(6, 6, Window_Size.x - 12, Window_Size.y - 12, { china_word5[12], china_word5[13], china_word5[14] }, { china_word5[15], china_word5[16], china_word5[17] }, TRUE);//BackGround Gradient
            if (BackGround_StyleCode)//彩虹条
            {
                In_DrawGradientRect(7, 7, (Window_Size.x - 14) / 2, 2, { china_word4[0] / 2, china_word4[1] / 2, china_word4[2] / 2 }, { china_word4[3] / 2, china_word4[4] / 2, china_word4[5] / 2 }, FALSE);
                In_DrawGradientRect(7 + (Window_Size.x - 14) / 2, 7, (Window_Size.x - 14) / 2, 2, { china_word4[3] / 2, china_word4[4] / 2, china_word4[5] / 2 }, { china_word4[6] / 2, china_word4[7] / 2, china_word4[8] / 2 }, FALSE);
                In_DrawGradientRect(7, 7, (Window_Size.x - 14) / 2, 1, { china_word4[0], china_word4[1], china_word4[2] }, { china_word4[3], china_word4[4], china_word4[5] }, FALSE);
                In_DrawGradientRect(7 + (Window_Size.x - 14) / 2, 7, (Window_Size.x - 14) / 2, 1, { china_word4[3], china_word4[4], china_word4[5] }, { china_word4[6], china_word4[7], china_word4[8] }, FALSE);
            }
        }
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        Vector2 GUI_Block(int X, int Y, int Length, const std::string & BlockText, int Width = 400, BOOL Block_Judgment = TRUE) noexcept//绘制按钮的区块
        {
            ...
            if (!BlockText.empty())In_DrawRect(X + 15, Y, In_DrawString(0, 0, BlockText, { 0,0,0 }, "Small Fonts", 14, 700) + 10, 3, { 10,10,10 });//文字后遮挡边框
        ...
        int GUI_Block_Panel(int X, int Y, int Width, int Length, std::string BlockText, std::vector<std::string>BlockText_, int& m_In_Block, int TextPos_X = 25) noexcept//区块_大区块选择器
        {
            if (m_In_Block < 0)m_In_Block = 0; else if (m_In_Block >= (int)BlockText_.size())m_In_Block = BlockText_.size() - 1;//范围限制
            In_DrawRect(X, Y, Width, Length, { 0,0,0 });//黑色外边框
            In_DrawRect(X + 1, Y + 1, Width - 2, Length - 2, { 60,60,60 });//白色外边框
            ...
            if (BlockText_.size() >= SHRT_MAX)
            {
                BlockText_.erase(BlockText_.begin() + SHRT_MAX, BlockText_.end());
            }

            for (int i = 0; i < (int)BlockText_.size(); ++i)//遍历坐标
            {
                ...
                if (m_In_Block == i)
                {
                    const auto SelPos = In_Animation<CreateClassName>(Y + 16 + 30.0f * i, 2);
                    In_DrawGradientRect(X + 2, (int)SelPos, Width - 4, 25, { 20,20,20 }, Global_EasyGUIColor / 4);
                    In_DrawGradientRect(X + 2, (int)SelPos + 1, Width - 4, 23, Global_EasyGUIColor / 4, { 20,20,20 });
                }
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        std::string GUI_Text(Vector2 BlockPos, int LineRow, const std::string & Text, Vector4 TextColor = { 200,200,200 }) noexcept//行文字
        {
        ...
        //--------------------------------------------------------------------------------------------------------------------------------------------------------
        BOOL GUI_Checkbox(Vector2 BlockPos, int LineRow, std::string Text, BOOL& m_CheckboxValue, Vector4 TextColor = { 200,200,200 }) noexcept//点击框
        {
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        BOOL GUI_Button(Vector2 BlockPos, int LineRow, const std::string& Text, BOOL& m_ButtonValue) noexcept//单击按钮
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return FALSE;//当无block则不进行绘制
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        BOOL GUI_Button_Small(Vector2 BlockPos, int LineRow, BOOL& m_ButtonValue, const std::string& Text = "...") noexcept//单击按钮(小号)
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return FALSE;//当无block则不进行绘制
            if (m_ButtonValue)Sleep(30);//更好地让其他循环线程接收
        ...
        ValueClass GUI_Slider(Vector2 BlockPos, int LineRow, std::string Text, ValueClass StartValue, ValueClass EndValue, ValueClass& m_SliderValue, std::string Unit = "", Vector4 TextColor = { 200,200,200 }) noexcept//滑条
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return FALSE;//当无block则不进行绘制
            ...
            {
                static const ValueClass ClassValueDetect = (ValueClass)0.1f;//检测是否是浮点值 (变量)
                if (DetectMousePos && (In_KeyEvent(VK_LEFT, TRUE) || In_KeyEvent(VK_UP, TRUE)))//当鼠标移动到滑条上方 按键反馈事件
                {
                    if (ClassValueDetect == 0)m_SliderValue--; else m_SliderValue -= (ValueClass)0.05f;//检测是否是浮点值
                }
            ...
            int SliderPos = (int)((float)(m_SliderValue - StartValue) / (float)(EndValue - StartValue) * 230);
            ...
            std::stringstream ss; ss << std::fixed << std::setprecision(3) << m_SliderValue; ss >> m_SliderValue;//只保留3位小数点后数
            ...
            In_DrawGradientRect(BlockPos.x + 55, BlockPos.y + 6 + 30 * LineRow, (int)In_Animation<CreateClassName>((float)SliderPos, 10.0f, { 0,230 }), 5, Global_EasyGUIColor / 3, Global_EasyGUIColor / 10, TRUE);//滑条减退动画 (动画0.8果冻效果)
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        template<class CreateClassName>
        int GUI_KeySelector(Vector2 BlockPos, int LineRow, int& m_KeySelectValue, std::string Mark = "") noexcept//按键选取按钮
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return FALSE;//当无block则不进行绘制
            if (m_KeySelectValue >= 0xCCCCCCCC)m_KeySelectValue = 0;//修复过量
            if (!Mark.empty())BlockPos.x -= 5;//让出空间
            ...
            if (!BoxMea)
            {
                ...
                default: DrawString_VK = "[" + std::to_string(m_KeySelectValue) + "]"; break;//如果什么都不是直接返回编码
                }
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        BOOL GUI_Tip(Vector2 BlockPos, int LineRow, const std::string& TipsString, Vector4 TextColor = { 200,200,200 }) noexcept//鼠标指针提示 (只支持单行字符,代码写在最后为了不被覆盖绘制)
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return FALSE;//当无block则不进行绘制
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        Vector4 GUI_ColorSelector(Vector2 BlockPos, int LineRow, Vector4& m_ColorValue) noexcept//颜色选择器
        {
            ...
            {
                const std::string Color_String_Copy__ = std::to_string(m_ColorValue.r) + ", " + std::to_string(m_ColorValue.g) + ", " + std::to_string(m_ColorValue.b);//要复制的字符串
                OpenClipboard(0);//打开剪切板
                EmptyClipboard();//清空剪切板
                const auto hHandle = GlobalAlloc(0, Color_String_Copy__.length() + 1);//分配内存
                strcpy_s((char*)GlobalLock(hHandle), Color_String_Copy__.length() + 1, Color_String_Copy__.c_str());
                SetClipboardData(1, hHandle);//设置剪切板数据
                GlobalUnlock(hHandle);//解除锁定
                CloseClipboard();//关闭剪切板
            }
            for (int Color_Bl = 0; Color_Bl <= 2; ++Color_Bl)
            {
                std::vector<int> UsedColor = { m_ColorValue.r,m_ColorValue.g,m_ColorValue.b };
            ...
            In_DrawString_Simple(BlockPos.x + 223, BlockPos.y - 2 + 30 * LineRow, "R: " + std::to_string(m_ColorValue.r), { 150,150,150 });
            In_DrawString_Simple(BlockPos.x + 269, BlockPos.y - 2 + 30 * LineRow, "G: " + std::to_string(m_ColorValue.g), { 150,150,150 });
            In_DrawString_Simple(BlockPos.x + 315, BlockPos.y - 2 + 30 * LineRow, "B: " + std::to_string(m_ColorValue.b), { 150,150,150 });
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        Vector4 GUI_ColorSelector_a(Vector2 BlockPos, int LineRow, Vector4& m_ColorValue) noexcept//颜色选择器(包含Alpha 透明度值)
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return { -1,-1,-1,-1 };//当无block则不进行绘制
            if (GetForegroundWindow() == EasyGUI_WindowHWND && In_MouseEventJudgment(BlockPos.x + 360, BlockPos.y - 6 + 30 * LineRow, 20, 20) && !Mouse_Slider_ && !Mouse_Move_ && In_KeyEvent(VK_LBUTTON, TRUE))//复制选取的颜色值
            {
                const std::string Color_String_Copy__ = std::to_string(m_ColorValue.r) + ", " + std::to_string(m_ColorValue.g) + ", " + std::to_string(m_ColorValue.b) + ", " + std::to_string(m_ColorValue.a);//要复制的字符串
                OpenClipboard(0);//打开剪切板
                EmptyClipboard();//清空剪切板
                const auto hHandle = GlobalAlloc(0, Color_String_Copy__.length() + 1);//分配内存
                strcpy_s((char*)GlobalLock(hHandle), Color_String_Copy__.length() + 1, Color_String_Copy__.c_str());
            ...
            for (int Color_Bl = 0; Color_Bl <= 3; ++Color_Bl)
            {
                std::vector<int> UsedColor = { m_ColorValue.r,m_ColorValue.g,m_ColorValue.b,m_ColorValue.a };
            ...
            In_DrawString_Simple(BlockPos.x + 177, BlockPos.y - 2 + 30 * LineRow, "R: " + std::to_string(m_ColorValue.r), { 150,150,150 });
            In_DrawString_Simple(BlockPos.x + 223, BlockPos.y - 2 + 30 * LineRow, "G: " + std::to_string(m_ColorValue.g), { 150,150,150 });
            In_DrawString_Simple(BlockPos.x + 269, BlockPos.y - 2 + 30 * LineRow, "B: " + std::to_string(m_ColorValue.b), { 150,150,150 });
            In_DrawString_Simple(BlockPos.x + 315, BlockPos.y - 2 + 30 * LineRow, "A: " + std::to_string(m_ColorValue.a), { 150,150,150 });
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        Vector3 GUI_PosSelector(Vector2 BlockPos, int LineRow, Vector3& m_PosValue) noexcept//坐标选择器
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return { -1,-1,-1 };//当无block则不进行绘制
            std::vector<float> UsedPos = { m_PosValue.x,m_PosValue.y,m_PosValue.z };
            for (int Pos_Bl = 0; Pos_Bl <= 2; ++Pos_Bl)
            {
                ...
                if (GetForegroundWindow() == EasyGUI_WindowHWND && DetectMousePos && !Mouse_Slider_ && !Mouse_Move_)//当最前端窗口为GUI窗口接收按钮事件 当鼠标移动到滑条上方 按键反馈事件
                {
                    if (In_KeyEvent(VK_LBUTTON, TRUE))UsedPos[Pos_Bl] += 0.03f;
                    else if (In_KeyEvent(VK_RBUTTON, TRUE))UsedPos[Pos_Bl] -= 0.03f;
                    if (In_KeyEvent(VK_LEFT, TRUE))UsedPos[Pos_Bl] += 1.f;
                    else if (In_KeyEvent(VK_RIGHT, TRUE))UsedPos[Pos_Bl] -= 1.f;
                    if (In_KeyEvent(VK_UP, TRUE))UsedPos[Pos_Bl] += 10.f;//快速变值
                    else if (In_KeyEvent(VK_DOWN, TRUE))UsedPos[Pos_Bl] -= 10.f;
                    if (In_KeyEvent(VK_DELETE, TRUE))UsedPos[Pos_Bl] = 0.f;
            ...
            std::stringstream Float_X_; Float_X_ << std::fixed << std::setprecision(2) << m_PosValue.x; Float_X_ >> m_PosValue.x;//只保留2位小数点后数
            std::stringstream Float_Y_; Float_Y_ << std::fixed << std::setprecision(2) << m_PosValue.y; Float_Y_ >> m_PosValue.y;
            std::stringstream Float_Z_; Float_Z_ << std::fixed << std::setprecision(2) << m_PosValue.z; Float_Z_ >> m_PosValue.z;
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        int GUI_List(Vector2 BlockPos, int StartLineRow, std::vector<std::string>LineString, int& m_InLine, int LimitLine = 0) noexcept//选择列表(缩小版 Panel)
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return FALSE;//当无block则不进行绘制
            if (m_InLine < 0)m_InLine = 0; else if (m_InLine >= (int)LineString.size())m_InLine = LineString.size() - 1;//范围限制
            if (!LimitLine)LimitLine = (int)LineString.size(); if (LineString.size() == 0)m_InLine = -1;
            ...
            if (LineString.size() >= SHRT_MAX)
            {
                LineString.erase(LineString.begin() + SHRT_MAX, LineString.end());
            }
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------
        template<class CreateClassName>
        std::string GUI_InputText(Vector2 BlockPos, int LineRow, std::string& m_String, std::string NormalText = "", BOOL HideString = FALSE) noexcept//字符串输入框 (英文数字 不支持UTF-8 最多30个字符)
        {
            if (BlockPos.x == 0 && BlockPos.y == 0)return "";//当无block则不进行绘制
            ...
            std::string DrawString = m_String;//绘制字符串
            if (HideString) { const auto StrSize = DrawString.size(); DrawString.clear(); for (size_t i = 0; i < StrSize; ++i)DrawString += "*"; }//隐藏字符串 (通常输入密码时使用)
                ...
                else if (IsInput)//在输入状态时
                {
                    ...
                    if (In_KeyEvent(VK_LCONTROL) || In_KeyEvent(VK_RCONTROL))//Ctrl键脚本
                    {
                        if (In_KeyEvent(0x43, TRUE))//Ctrl + C 复制
                        {
                            OpenClipboard(0);//打开剪切板
                            EmptyClipboard();//清空剪切板
                            const auto hHandle = GlobalAlloc(0, m_String.length() + 1);//分配内存
                            strcpy_s((char*)GlobalLock(hHandle), m_String.length() + 1, m_String.c_str());
                            SetClipboardData(1, hHandle);//设置剪切板数据
                            GlobalUnlock(hHandle);//解除锁定
                            CloseClipboard();//关闭剪切板
                        }
                    ...
                    for (int i = 0x8; i < 0xFE; ++i)//VK键码遍历 (检测按下了什么键)
                    {
                        ...
                        if (In_KeyEvent(i, TRUE))
                        {
                            ...
                            case VK_DELETE:m_String.clear(); break;//清除全部字符
            ...
            if (!IsInput && DrawString.empty())In_DrawString(BlockPos.x + 63, BlockPos.y - 2 + 30 * LineRow, NormalText, { 50,50,50 }, Global_EasyGUIFont, Global_EasyGUIFontSize);//默认显示文字
            else {//已输入的文字
